...
1. **Decide first: should this animate at all?** Read `$HOME/.agents/skills/design-engineering/references/motion/animation-decision-framework.md`. If the answer is no, return a one-line "remove this animation" recommendation and stop. Most over-animated UI is the wrong question.
...
> Per-agent identity. Inherits from the root agent's instructions — this section narrows that to motion specifically.
