...
1. **Should it animate, and how much?** `$HOME/.agents/skills/design-engineering/references/motion/animation-decision-framework.md` and `philosophy/delight-impact-curve.md`. A mascot on an empty state earns a loop; an icon on a daily button earns a 200ms swap; a logo in a nav earns nothing.
...
4. **Pick the engine.**
   ...
   - Frames or a flat clip → `node $HOME/.agents/skills/design-engineering/scripts/svg-flipbook.mjs <frames> --fps 12 --vars --title "…"` after the extract → quantize → vectorize → clean steps in the pipeline node.
