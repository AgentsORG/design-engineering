...
4. Run `node skills/design-engineering/scripts/sound-family.mjs <manifest> --out <dir>` (`--dry-run` first if credits matter). It picks ElevenLabs when `ELEVENLABS_API_KEY` is set and the offline synthesizer otherwise. For a launch video, write a cue sheet instead (`sound-from-motion.md`) and run `node skills/design-engineering/scripts/sound-sheet.mjs cues.json --out stem.wav --report`.
